...
NumbersBinnerAttributes::NumbersBinnerAttributes()
{
    ...
    m_externalQMLLocations.push_back({true, false, 10,120});
...
QJsonObject NumbersBinnerIdea::save() const
{
    ...
    for(size_t i = 0; i < m_binBounds.size(); i++)
    {
...
void NumbersBinnerIdea::load(const QJsonObject &obj)
{
    ...
    if(!v.isUndefined())
    {
        ...
        {
            m_binBounds.push_back(boundsArray.at(i).toDouble());
        }
...
{
    emit binBoundsChanged(m_binBounds);
    if(status() == IdeaStatus::Working || status() == IdeaStatus::DisplayWarning)
    {
...
void NumbersBinnerIdea::validate()
{
    ...
    if(v_nums)
    {
        ...
        else
        {
            ...
            bool valid = true;
            for(size_t i = 1; i < m_binBounds.size(); i++)
            {
            ...
            {
                if(m_nums.lock().get()->numbers().size() == 0)
                {
...
void NumbersBinnerIdea::compute()
{
    ...
    std::vector<double> input = m_nums.lock().get()->numbers();
    ...
    if(m_binBounds.size() == 1)
    {
        ...
        else
        {
            if(lowBound < m_gridBounds[0] || upBound > m_gridBounds[1])
                setStatus(IdeaStatus::DisplayWarning,"Display Error?", "The grid bounds might not contain all the data. "
    ...
    else
    {
        if(m_autoFitGrid)
        {
            double width = (m_binBounds[m_binBounds.size()-1] - m_binBounds[0]) / (m_binBounds.size() - 1);
            if(lowBound > m_binBounds[0] - width)
                lowBound = m_binBounds[0] - width;
            if(upBound < m_binBounds[m_binBounds.size()-1] + width)
                upBound = m_binBounds[m_binBounds.size()-1] + width;
    ...
    if(width != 0)
        m_binWidths.push_back(width);
    else
        m_binWidths.push_back(1);
    for(int i = 1; i < size; i++)
    {
        m_binWidths.push_back((int) 450 *(m_binBounds[i] - m_binBounds[i-1]) / (m_gridBounds[1] - m_gridBounds[0]));
    }
    ...
    m_binBounds.push_back(1.7976931348623157E+308);
    ...
    for(int i = 0; i < inputSize; i++)
    {
        ...
        m_dotInitialPositions.push_back(initPos);
        ...
        m_dotFinalPositions.push_back({getPositionForBoundLine(j -1) + xShift, static_cast<double>(qMax(296 - yShift, 6))});
    }

    //Remove maximum to avoid having to check if j is valid in loop.
    m_binBounds.pop_back();
...
std::vector<double> NumbersBinnerIdea::binBounds() const
{
    return m_binBounds;
}
...
std::vector<double> NumbersBinnerIdea::gridBounds() const
{
    return m_gridBounds;
}

void NumbersBinnerIdea::setBinBounds(std::vector<double> bounds)
{
...
void NumbersBinnerIdea::setBoundsByButton()
{
    ...
    if(m_numBins == 2)
    {
        newBounds.push_back(m_upBinBoundary + m_lowBinBoundary / 2.0);
        setBinBounds(newBounds);
        validate();
    }
    else
    {
        ...
        {
            newBounds.push_back(m_lowBinBoundary + i * jumpSize);
        }
        setBinBounds(newBounds);
        validate();
    }
